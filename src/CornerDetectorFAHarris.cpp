...
{
CornerDetectorFAHarris::CornerDetectorFAHarris():
...
bool CornerDetectorFAHarris::isFiltered(const dvs_msgs::Event &e)
{
...
bool CornerDetectorFAHarris::isCorner(const dvs_msgs::Event &e)
{
...
bool CornerDetectorFAHarris::addNewEventToTimeSurface(const dvs_msgs::Event &e)
{
    double et = e.ts.toSec();
    int ex = e.x;
    int ey = e.y;
...
bool CornerDetectorFAHarris::isCornerWithoutFilter(const dvs_msgs::Event &e)
{
    // Return if too close to the border
    if (e.x < window_size_ or e.x >= sensor_width_ - window_size_ or e.y < window_size_ or e.y >= sensor_height_ - window_size_)
    {
...
bool CornerDetectorFAHarris::isCornerCandidate(const dvs_msgs::Event &e)
{
    int ex = e.x;
    int ey = e.y;
...
bool CornerDetectorFAHarris::isCornerCandidateRefined(const dvs_msgs::Event &e)
{
...
double CornerDetectorFAHarris::getHarrisScore(int img_x, int img_y, bool polarity)
{
...
bool CornerDetectorFAHarris::checkPatch(const dvs_msgs::Event &e)
{
    patch_ = Eigen::MatrixXi::Constant(9, 9, 0);
...
Eigen::MatrixXi CornerDetectorFAHarris::getPatch()
{
...
int CornerDetectorFAHarris::factorial(int n) const
{
    if (n > 1)
    {
...
int CornerDetectorFAHarris::pasc(int k, int n) const
{
    if (k >= 0 && k <= n)
    {
